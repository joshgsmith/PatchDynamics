...
```{r}
size_fq<- read.csv("size_year.csv")
...
```{r}
DatU.raw <- read.csv('size_year.csv')
...
#Load Data
```{r}
dat = read.csv("size_year.csv")
dat2 = read.csv("den_year.csv")
saveresults = 0
loadresults = 1
```
...
Gr = c(0.8956,0.9294,0.5958,0.2500,0.0853,0.0852,0.0848,0.0859,0.0831) #Growth Probabilities

#avg size across years
...
if (loadresults==1){
  ...
  if(saveresults == 1){
    fit$save_object(file = "Urchin_fit.RDS")
    rm(fit)
...
#Examine results
```{r}
# (note: using final model, Model 8 = shift in all variables)
