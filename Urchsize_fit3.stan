...
parameters {
  real<lower=0,upper=10> phi ;     // inverse scale param for gamma distribution
  real<lower=0> theta[2] ;        // params for precision of dirichlet dist. 
  real<lower=0,upper=15> sigR[2] ;// variation in log recruitment
  real<lower=0,upper=15> sigS[2] ;// variation in log hazards
  real<lower=0,upper=15> sigP[2]; // variation in logit detection prob
  vector<lower=0>[Nyrs] epsR ;    // random effect, recruitment by year
  vector<lower=0>[Nyrs] epsS ;    // random effect, survival (log hazards) by year
  vector<lower=0>[Nyrs] epsP ;    // random effect, detection prob (logit) by year
  real<lower=0> Beta ;            // par2, logit fxn of Prob detection  
  real gamma0 ;                   // baseline hazards (for survival)
  real<lower=0> R0 ;              // baseline recruitment (log)
  // real<lower=0> N0 ;              // initial abundance
} 
...
transformed parameters {
  ...
  // Loop through post-switchpoint years
  for (t in (YrB+1):Nyrs){
    matrix[Nsz,Nsz] Mt;             // matrix, year t
    vector[Nsz] Rt      ;           // recruitment vector, yr t
    vector[Nsz] St      ;           // survival vector, yr t
    S[t] = exp(-exp(gamma0 - sigS[2] * epsS[t])) ;
    St = rep_vector(S[t],Nsz)  ; 
    R[t] = exp(R0 + sigR[2] * epsR[t]) ;
    Rt = Zeros ;
    Rt[1] = R[t] ;
    P[t] = inv_logit(lgtPD + sigP[2] * epsP[t]) ;
    Mt = makemat(Nsz,Gr,St) ;
    n[t] = Mt * (n[t-1] + Rt) + .00001 ;
    N[t] = sum(n[t]) ;
    d[t] = n[t] .* P[t] ;
...
// Section 5. Derived parameters and statistics 
 generated quantities {
  ...
  Pmn_post  = mean(Pmax[(YrB+1):(Nyrs-1)]); 
