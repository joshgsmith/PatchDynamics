...
data {
  ...
  real sigP_est[2];              // variation in logit detection prob estimated
  real xi ;                      // proportional adjustment to variation in detection prob
}
//
transformed data{
  real sigP[2];
  sigP[1] = sigP_est[1] * xi ;
  sigP[2] = sigP_est[2] * xi ;
}
...
parameters {
  ...
  vector<lower=-2.6,upper=2.6>[Nyrs] epsR ; // random effect, recruitment by year
  vector<lower=-2.6,upper=2.6>[Nyrs] epsS ; // random effect, survival (log hazards) by year
  vector<lower=-2.6,upper=2.6>[Nyrs] epsP ; // random effect, detection prob (logit) by year
...
// Section 5. Derived parameters and statistics 
 generated quantities {
  ...
  for(i in 1:Nszobs){
    log_lik[i] = dirichlet_lpdf( Obs_Sz[i] | (d[Sy[i]] ./ D[Sy[i]]) * (theta[1]*pow(SzDst_N[i],theta[2]))) ;
  }
  for(i in 1:Nsurvey){
    log_lik[Nszobs+i] = gamma_lpdf(Density[i] | D[Dy[i]] * phi, phi) ;
